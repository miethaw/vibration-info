                    ...
                    <Menu iconShape="circle">
                        {
                            MenuItems.filter(f => (f.siteIds && f.siteIds.includes(siteInfo.siteId)) || !f.siteIds).map((d, i) => {
                                ...
                                return (
                                    <MenuItem onClick={() => { if (window.location.pathname != d.routeTo) window.location = d.routeTo; }} key={i} popperarrow={true} className={`d-flex ${openSidebar === true ? '' : ' mx-3 my-2'}`}
                                        style={{ width: 240, height: 90, background: openSidebar ? 'none' : (window.location.pathname != d.routeTo) ? '#293365' : '#0e5690', borderRadius: 10 }}

                                        icon={
                                        <div className="d-flex justify-content-center align-items-center w-100 h-100" style={{ borderRadius: '50%', background: (window.location.pathname != d.routeTo) ? 'none' : '#0e5690' }}>
                         
                                            <CustomPopover
                                                {...props}
                                                title={d.title}
                                                active={(window.location.pathname != d.routeTo) }
                                            >
